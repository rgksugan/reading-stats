(function(_, d3, moment) {
  ...
  function timeline(books) {
    var node = document.getElementById("timeline");
    var margin = { top: 20, right: 0, bottom: 50, left: 80 };
    var width =
      parseInt(window.getComputedStyle(node.parentNode).getPropertyValue("width")) - margin.left - margin.right;
    var domain = d3.extent(_.map(books, "read"));
    ...
    card
      .attr("title", function(d) {
        return d.read.format("D MMM YYYY") + "  " + stars(d.rating) + "  " + d.title + " - " + d.author;
      })
  ...
  function timelineSmall(id, books, range) {
    var node = document.getElementById(id);
    var margin = { top: 0, right: 10, bottom: 20, left: 10 };
    var width =
      parseInt(window.getComputedStyle(node.parentNode).getPropertyValue("width")) - margin.right - margin.left;
    var height = 60;
    var svg = d3
      .select(node)
      .attr("width", width + margin.left + margin.right)
    ...
    var readInRange = _.filter(books, function(b) {
    ...
    svg
      ...
      .call(
        d3
          .axisBottom(x)
          .tickValues(
            _.map(_.range(domain[0].year(), domain[1].year() + 1), function(year) {
    ...
    cell.append("title").text(function(d) {
      return d.data.read.format("MMM YYYY") + "  " + stars(d.data.rating) + "  " + d.data.title;
    });
  }

  function distribution(books) {
    ...
    var distribution = _.mapValues(_.groupBy(books, "rating"), function(l) {
      return l.length;
    });
  ...
  function timelinePublications(id, books, height, domain) {
    var node = document.getElementById(id);
    ...
    var published = _.filter(books, function(b) {
      return b.published.isValid() && b.published >= domain[0] && b.published < domain[1];
    });
    ...
    cell.append("title").text(function(d) {
      if (d) {
        return d.data.published.format("YYYY") + "  " + stars(d.data.rating) + "  " + d.data.title;
      }
    });
  }
  ...
  function pageCount(books) {
    ...
    var readWithCount = _.filter(books, function(b) {
      return b.pageCount > 0;
    });
    var domain = [0, _.max(books, "pageCount").pageCount];
    var x = d3
      .scaleLinear()
      .domain(domain)
      .rangeRound([0, width])
    ...
    cell.append("title").text(function(d) {
      return d && d.data.pageCount + "  " + stars(d.data.rating) + "  " + d.data.title;
    });
  }

  function pagePerYear(books) {
  ...
  function tags(readBooks, shelves) {
    ...
    d3.select(node)
      ...
      .html(function(d) {
        return d + ' <span class="tag-count">' + (filter(d) && filter(d).length) + "<span>";
      });
  ...
  function bookPerShelf(books, shelves, shelfColor) {
    ...
    var distribution = _.chain(books)
      .map("bookshelves")
      .flatten()
      .without("", "own")
      .groupBy()
  ...
  function timelineShelf(id, books, shelves, shelfColor, range) {
    var node = document.getElementById(id);
    ...
    var data = _.chain(ticks)
      .map(function(month) {
        var distribution = _.chain(books)
          .filter(function(b) {
            return month.isSame(b.read, "month") || month.isBetween(b.started, b.read);
          })
          .map(function(b) {
  ...
  function authors(books) {
    ...
    cell.append("title").text(function(d) {
      return d.data[1].length + " " + d.data[0] + "\n\n" + _.map(d.data[1], "title").join("\n");
    });
  }

  var linkToBook = function(book) {
    return `https://www.amazon.com/gp/search?ie=UTF8&tag=rgksugan-21&linkCode=ur2&linkId=5bbd98855da3498ae3249f371acb0efc&camp=1789&creative=9325&index=books&keywords=${encodeURIComponent(
      book.fullTitle + " " + book.author
    )}`;
  };
  ...
  fetch("./data/books.csv")
    ...
    .then(function(body) {
      var books = Papa.parse(body, {
        delimiter: ",",
        header: true
      }).data;

      var read = _.chain(books)
        ...
        .map(function(book) {
          ...
          var bookshelves = [];
          if (book["Bookshelves"] != 0) {
            bookshelves = book["Bookshelves"].split(", ");
          }
          if (book["ISBN13"]) {
            var matches = book["ISBN13"].match(/^="(.*)"$/gm);
            if (matches && matches[0]) {
              book.isbn = matches[0].replace("=", "").toString();
            }
          }
          return {
      ...
      var shelves = _.chain(read)
        .filter(function(book) {
          return book.bookshelves.length > 0;
        })
        .map("bookshelves")
        .flatten()
        .uniq()
      ...
      var shelfColor = (function() {
        var color = d3.scaleSequential(d3.interpolateRainbow).domain([0, shelves.length]);
      ...
      timeline(read);
      timelineSmall("timeline-small-1", read, [year("2010"), year("2015")]);
      timelineSmall("timeline-small-2", read, [year("2015"), year("2020")]);
      distribution(read);
      var breakAt = year("1970");
      timelinePublications("timeline-publication-too-old", read, 30, [year("-1000"), year("1800")]);
      timelinePublications("timeline-publication-old", read, 50, [year("1800"), breakAt.clone()]);
      timelinePublications("timeline-publication-current", read, 75, [breakAt.clone(), moment()]);
      pageCount(read);
      pagePerYear(read);
      bookPerYear(read);
      authors(read);
      bookPerShelf(read, shelves, shelfColor);
      timelineShelf("timeline-shelf-1", read, shelves, shelfColor, [year("2010"), year("2015")]);
      timelineShelf("timeline-shelf-2", read, shelves, shelfColor, [year("2015"), year("2020")]);
      tags(read, shelves);
    });
})(_, d3, moment);
